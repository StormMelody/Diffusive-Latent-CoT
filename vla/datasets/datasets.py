...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.action_tokenizer is None:
            ...
            gpt_values_noise = ""
            noise_min = np.array([-0.052826, -0.001857, -0.033522, -0.000465, -0.596363, -0.002859])  
            noise_max = np.array([-0.035852, 0.000827, -0.028384, -0.000126, -0.572663, -0.001982])   
            final_noise = np.random.uniform(0, 1, size=6)  
            final_noise = noise_min + (noise_max - noise_min) * final_noise  
            gt_noise = np.zeros_like(act)
            gt_noise[:6] = final_noise 
            for act in action:
                gpt_values_noise += self.action_tokenizer(act+gt_noise)
            conversation_noise = [
                {"from": "human", "value": f"What action should the robot take to {lang}?"},
                {"from": "gpt", "value": f"<BOD><EOD>{gpt_values_noise}"},
            ]
        ...
        prompt_builder = self.prompt_builder_fn("openvla")
        for turn in conversation_noise:
            prompt_builder.add_turn(turn["from"], turn["value"])
        # Tokenize (w/ `base_tokenizer`)
        input_ids_noise = self.base_tokenizer(prompt_builder.get_prompt(), add_special_tokens=True).input_ids

        labels = list(input_ids)

        # Tensorize =>> Run Image Transform to get `pixel_values` =>> Return
        #   =>> IMPORTANT :: IF WE'RE USING HF LLM.forward(..., labels=labels), SHIFTING HAPPENS _INSIDE_ MODEL!
        input_ids, labels, input_ids_noise  = torch.tensor(input_ids), torch.tensor(labels), torch.tensor(input_ids_noise)
        pixel_values = {}
        front_pixel_values = self.image_transform(front_img)
        ...
        if wrist_left_img is not None:
            wrist_left_pixel_values = self.image_transform(wrist_left_img)
        ...
        return dict(pixel_values=pixel_values, input_ids=input_ids, labels=labels, dataset_name=dataset_name, actions=action, action_masks=action_mask, input_ids_noise=input_ids_noise, proprio = proprio)
