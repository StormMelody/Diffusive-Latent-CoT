...
import enum
from util import discretized_gaussian_log_likelihood, normal_kl
...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, past_key_values=None, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        """
    ...
    def ddim_sample(
        ...
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "past_key_values": out['extra'].past_key_values}
...
class LabelEmbedder(nn.Module):
    ...
    def token_drop(self, conditions, force_drop_ids=None):
        ...
        if force_drop_ids is None:
            drop_ids = th.rand(conditions.shape[0], device=conditions.device) < self.dropout_prob
        else:
            drop_ids = force_drop_ids == 1
        self.uncondition = self.uncondition.to(conditions.device)
        conditions = th.where(drop_ids.unsqueeze(1).unsqueeze(1).expand(conditions.shape[0], *self.uncondition.shape), self.uncondition, conditions)
        return conditions
...
class ActionEmbedder(nn.Module):
    def __init__(self, action_size, hidden_size):
        ...
        self.mlp = Mlp(in_features=action_size,
